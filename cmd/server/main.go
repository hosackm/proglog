...
//    -import-path api -proto api/log.proto localhost:55476 log.v1.Log/Produce
//
// Consume the log record
// grpcurl -plaintext -d '{"offset": 0}' -import-path api -proto api/log.proto \
// localhost:55476 log.v1.Log/Consume | jq -r .record.value | base64 --decode
//
// For streaming service you can start the consume stream with:
// grpcurl -plaintext -import-path api -proto api/log.proto localhost:55476 log.v1.Log/ConsumeStream
// then you can grpcurl to the Produce service
//
// grpcurl -plaintext -d '{"record": {"value": "dmFsdWUgMx=="}}' -import-path api -proto api/log.proto localhost:55476 log.v1.Log/Produce
